    ...
    if [ -d "${proc_dir}fd" ]; then
        ...
        for fd in "${proc_dir}fd"/*; do
            ...
            if [ -L "$fd" ] && [ -e "$fd" ]; then
                ...
                # (Если нужно выводить абсолютно всё, включая сокеты, удалить следующие 3 строки)
...
```
## Выполнение из под пользователя
<img width="859" height="645" alt="изображение" src="https://github.com/user-attachments/assets/d175d582-bf33-4c79-ae02-e4d422e782ff" />
